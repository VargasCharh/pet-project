...
export default function ModalUpdatePostBtn({ companies, post }: AddModalProps): JSX.Element {
  const {  updateHandler } = useForumHook();
  ...
  return (
    <>
      <button
        data-modal-target={`updateQuestionModal-${post.id}`}
        data-modal-toggle={`updateQuestionModal-${post.id}`}
      ...
      <div
        id={`updateQuestionModal-${post.id}`}
        tabIndex="-1"
        aria-hidden="true"
        ...
        <div className="relative w-full max-w-md max-h-full">
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <button
              ...
              data-modal-hide={`updateQuestionModal-${post.id}`}
            >
              <svg
                aria-hidden="true"
                className="w-5 h-5"
